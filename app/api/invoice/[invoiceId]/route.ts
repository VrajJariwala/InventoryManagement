...
export async function GET(
  ...
  pdf.text(
    `${String.fromCharCode(8377)} ${formatCurrency({ amount: data.invoiceItemRate, currency: data.currency as any })}`,
    130,
    110
  );
  pdf.text(
    `${String.fromCharCode(8377)} ${formatCurrency({ amount: data.total, currency: data.currency as any })}`,
    160,
    110
  );
